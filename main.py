...
one_step_vars = [tf.Variable(tf.zeros_like(var.iniyialized_value()), trainable=False) for var in trainable_vars]


accumed_vars = [tf.Variable(tf.zeros_like(var.iniyialized_value()), trainable=False) for var in trainable_vars]
...
if IS_TRAINING:
    in_shape = [None, N_FRAMES, 240, 320]
else:
    in_shape = [1, N_FRAMES, 240, 320]
